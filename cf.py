...
class CFWorkerCLI:
    ...
    def show_header(self, title="CF Worker CLI", subtitle="Cloudflare Worker Deployment Tool"):
    ...
    def list_accounts(self):
        ...
        for i, account in enumerate(self.accounts, 1):
            api_key_preview = account["global_api_key"][:8] + "..." + account["global_api_key"][-4:]
    ...
    def remove_account(self):
        ...
        try:
            ...
            if 1 <= choice <= len(self.accounts):
                ...
                self.save_accounts()
                self.show_success(f"Account {removed_account['email']} removed successfully!")
            else:
    ...
    def add_github_url(self):
        ...
        if len(self.github_urls) == 1:  # Jika ini URL pertama, otomatis set sebagai default
    ...
    def remove_github_url(self):
        ...
        try:
            ...
            if 1 <= choice <= len(self.github_urls):
                ...
                self.save_github_urls()
                self.show_success(f"GitHub URL '{removed_item['name']}' removed successfully!")
    ...
    def set_default_github_url_menu(self):
        ...
        print("-" * 50)
        for i, item in enumerate(self.github_urls, 1):
            current_indicator = " ← CURRENT DEFAULT" if item.get("is_default", False) else ""
        ...
        try:
            ...
            if 1 <= choice <= len(self.github_urls):
                ...
                self.show_success(f"Default GitHub URL set to: {self.github_urls[choice - 1]['name']}")
            else:
                self.show_info("Cancelled")
        except:
            self.show_error("Invalid choice")
    ...
    def select_github_url(self):
        ...
        try:
            choice = input(f"\nSelect URL [1-{len(self.github_urls)} or Enter for default]: ").strip()
            if not choice:
                return self.get_default_github_url()
        ...
        if failed > 0:
            ...
            for result in results:
                if not result["success"]:
                    print(f"• {result['worker']} on {result['account']}: {result['error']}")
